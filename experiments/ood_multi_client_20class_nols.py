#!/usr/bin/env python3
"""
OOD Multi-Client: 20 classes per client, NO Label Smoothing
=============================================================
5 clients, each trained on 20 classes of CIFAR-100 (no label smoothing).
- Client 0: classes 0-19
- Client 1: classes 20-39
- Client 2: classes 40-59
- Client 3: classes 60-79
- Client 4: classes 80-99
Server: DINOv2 on all 100 classes.
OOD for each client: the other 80 classes.
Without label smoothing, client should be more overconfident on OOD.
"""
...
def main():
    ...
    for cid in range(N_CLIENTS):
        ...
        if cid == 0:
            ...
            print(f"RESULTS")
            print(f"{'='*80}")
        print(f"\n--- Client {cid} (classes {cid*CLASSES_PER_CLIENT}-{(cid+1)*CLASSES_PER_CLIENT-1}) ---")
