#!/usr/bin/env python3
"""
OOD Multi-Client: 30 RANDOM classes per client, NO Label Smoothing
===================================================================
5 clients, each trained on 30 randomly selected classes of CIFAR-100.
Classes may overlap between clients (realistic scenario).
Server: DINOv2 on all 100 classes.
OOD for each client: the other 70 classes it hasn't seen.
"""
...
def train_client(model, train_sub, epochs=60):
    ...
    crit = nn.CrossEntropyLoss()  # No label smoothing
...
def main():
    t0 = time.time()
    print("=" * 80)
    print("OOD Multi-Client: 30 Random Classes per Client, No Label Smoothing")
    print("=" * 80)
    ...
    all_classes = list(range(N_TOTAL))
    client_classes = {}
    for cid in range(N_CLIENTS):
        selected = sorted(random.sample(all_classes, CLASSES_PER_CLIENT))
        client_classes[cid] = set(selected)
        print(f"  Client {cid}: {CLASSES_PER_CLIENT} random classes: {selected[:5]}...{selected[-3:]}")
    ...
    for cid in range(N_CLIENTS):
        ...
        print(f"  Client {cid} ({CLASSES_PER_CLIENT} classes):")
    ...
    for cid in range(N_CLIENTS):
        ...
        if cid == 0:
            ...
            print(f"RESULTS")
            print(f"{'='*80}")
        print(f"\n--- Client {cid} ({CLASSES_PER_CLIENT} classes, OOD={N_TOTAL-CLASSES_PER_CLIENT}) ---")
