#!/usr/bin/env python3
"""
v8.2c: Improved Client Training + Calibration + Deferral Comparison
====================================================================
Same as v8.2b but with better training techniques:
1. Label Smoothing (0.1)
2. Mixup (alpha=0.2)
3. Longer training + Warmup (5ep linear warmup)

Goal: See if better calibration changes ConfTh vs L2H dynamics.
"""
...
def main():
    ...
    # 7) L2H rejectors (joint training)
    print("\n[7] Training L2H rejectors (joint, c1=1)...")
    rejectors = []; l2h_heads = []
    for cid in range(5):
    ...
    # 8) Evaluate
    print("\n[8] Computing scores...")
    val_data = {}; test_data = {}; val_conf_data = {}; test_conf_data = {}
    for cid in range(5):
        # L2H scores (use jointly-trained server head)
        val_data[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, l2h_heads[cid], rejectors[cid], val_sub)
        test_data[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, l2h_heads[cid], rejectors[cid], tst_sub)
        # ConfTh scores (use original server head)
        val_conf_data[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, srv_heads[cid], rejectors[cid], val_sub)
        test_conf_data[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, srv_heads[cid], rejectors[cid], tst_sub)
    ...
    # Server accuracy
    print("\n--- Server Accuracy (per-client heads) ---")
    for cid in range(5):
        _, sp, y, _, _ = test_conf_data[cid]
        print(f"  {NAMES[cid]}: {100*np.mean(sp==y):.2f}%")

    for rate in DEFERRAL_RATES:
        print(f"\n{'='*80}")
        print(f"  Deferral Rate = {rate}%")
        print(f"{'='*80}")
        print(f"  {'Client':<15} {'L2H':>8} {'ConfTh':>8} {'Random':>8} {'L2H-CT':>8}")
        print(f"  {'-'*15} {'-'*8} {'-'*8} {'-'*8} {'-'*8}")
        all_l, all_c, all_r = [], [], []
        for cid in range(5):
            # L2H
            vs = val_data[cid][3]; ts = test_data[cid]
            th = find_threshold(vs, rate)
            l2h_acc = apply_threshold(ts[0], ts[1], ts[2], ts[3], th)
            # ConfTh
            vc = val_conf_data[cid][4]; tc = test_conf_data[cid]
            cth = find_conf_threshold(vc, rate)
            ct_acc = apply_conf_threshold(tc[0], tc[1], tc[2], tc[4], cth)
            # Random
            rnd = random_defer(tc[0], tc[1], tc[2], rate)
            diff = l2h_acc - ct_acc
            all_l.append(l2h_acc); all_c.append(ct_acc); all_r.append(rnd)
            print(f"  {NAMES[cid]:<15} {100*l2h_acc:>7.2f}% {100*ct_acc:>7.2f}% {100*rnd:>7.2f}% {100*diff:>+7.2f}%")
        print(f"  {'-'*15} {'-'*8} {'-'*8} {'-'*8} {'-'*8}")
        print(f"  {'AVERAGE':<15} {100*np.mean(all_l):>7.2f}% {100*np.mean(all_c):>7.2f}% {100*np.mean(all_r):>7.2f}% {100*(np.mean(all_l)-np.mean(all_c)):>+7.2f}%")
