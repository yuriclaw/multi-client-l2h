"""
Two-Stage L2H v7.5: Threshold-based Deferral (Realistic Evaluation)
====================================================================
Key change: Find deferral threshold on VALIDATION set, apply on TEST set.
...
Also reports: Server-only accuracy (per-client + universal)
"""
...
def get_client_data():
    ...
    for cid, cfg in CLIENT_CONFIGS.items():
        ...
        te_total = min(N_VAL + N_TEST, len(te))
        te_all_idx = random.sample(range(len(te)), te_total)
        ...
        clients[cid] = {
            ...
            'train': Subset(tr, tr_idx), 'val': Subset(te, val_idx), 'test': Subset(te, te_idx),
        }
...
def main():
    t_start = time.time()
    print("=" * 100)
    print("L2H v7.5: Threshold-based Deferral (Realistic Evaluation)")
    print("=" * 100)
    ...
    for bb_name in BACKBONES:
        ...
        for rate in eval_rates:
            ...
            for cid, info in clients.items():
                ...
                val_scores_l2h = get_scores_l2h(l2h_rej[cid], client_models[cid], info['val'])
                thresh_l2h = find_threshold(val_scores_l2h, rate)
                test_scores_l2h = get_scores_l2h(l2h_rej[cid], client_models[cid], info['test'])
