"""
Two-Stage L2H v7.4: MobileNetV2 Full FT Client + DINOv2 Server (2-layer MLP head)
===================================================================================
...
Rejector: MLP on client hidden features
"""
...
def main():
    t_start = time.time()
    print("=" * 100)
    print("L2H v7.4b: Per-Client vs Universal Server Head Comparison")
    print("=" * 100)
    ...
    for bb_name in BACKBONES:
        ...
        for rate in eval_rates:
            ...
            for cid, info in clients.items():
                ...
                # L2H — same rejector, but server uses universal head
                s = get_scores_l2h(l2h_rej[cid], client_models[cid], info['test'])
                r_l2h = system_acc(s, rate, cp, sp_uni, labels)
                
                # GK — same finetuned client, but server uses universal head
                s = get_scores_gk(gk_client[cid], info['test'])
                cp_gk, _ = get_preds(gk_client[cid], info['test'])
                r_gk = system_acc(s, rate, cp_gk, sp_uni, labels)
                
                # ConfTh — universal server
                s = get_scores_conf(client_models[cid], info['test'])
                r_conf = system_acc(s, rate, cp, sp_uni, labels)
                
                # Random — universal server
                r_rand = random_acc(rate, cp, sp_uni, labels)
