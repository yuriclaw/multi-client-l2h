"""
Two-Stage L2H v7.2: MobileNetV2 Pretrained Client + 3 Server Backbones
=======================================================================
...
Rejector: MLP on client hidden features only (no raw image CNN)
"""
...
N_TEST = 1000
DATA_DIR = '/tmp/data'
CLIENT_EPOCHS = 50  # Only training the linear head, converges fast
HEAD_EPOCHS = 200
STAGE2_EPOCHS = 100
BATCH_SIZE = 128
...
class ClientMobileNetV2(nn.Module):
    def __init__(self, n_classes):
        ...
        self.features = base.features  # Frozen backbone
...
def train_client(n_classes, train_ds, test_ds):
    ...
    # Only optimize classifier (backbone frozen)
    opt = optim.Adam(model.classifier.parameters(), lr=1e-3, weight_decay=1e-4)
...
def main():
    t_start = time.time()
    print("=" * 100)
    print("L2H v7.2: MobileNetV2 Client + 3 Server Backbones")
    print("=" * 100)
    
    print("\n[1] Loading data...")
    clients = get_client_data()
    
    print(f"\n[2] Training client MobileNetV2 linear head ({CLIENT_EPOCHS}ep)...")
    client_models = {}
