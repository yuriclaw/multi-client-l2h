#!/usr/bin/env python3
"""
v8.2d: Per-Client vs Universal (Rejector + Server Head)
========================================================
Same improved training as v8.2c, but adds:
- Universal server head (trained on all 25K samples)
- Universal rejector (with per-client projection layers)
- Full comparison: Per-L2H vs Uni-L2H vs ConfTh vs Random
"""
...
def main():
    ...
    # 6b) Universal server head
    print("\n[6b] Training UNIVERSAL server head (all 25K samples)...")
    all_train_indices = []
    for ci in client_train_idx:
        all_train_indices.extend(ci)
    all_train_sub = Subset(train_full, all_train_indices)
    uni_srv_head = ServerHead(server.feat_dim, N_CLASSES)
    uni_srv_head = train_server_head(server, uni_srv_head, all_train_sub)
    ...
    # 8) Universal L2H rejector (joint training)
    print("\n[8] Training universal L2H rejector (joint, c1=1)...")
    client_hidden_dims = {cid: models[cid].hidden_dim for cid in range(5)}
    uni_rejector = UniversalRejector(client_hidden_dims)
    uni_rejector, uni_l2h_head = train_universal_rejector_joint(
        uni_rejector, models, TEST_TFS, server, uni_srv_head,
        client_train_subs, client_hidden_dims, epochs=30, c_1=1.0)

    # 9) Evaluate
    print("\n[9] Computing scores...")
    val_pc = {}; test_pc = {}  # per-client L2H
    val_uni = {}; test_uni = {}  # universal L2H
    val_conf = {}; test_conf = {}  # ConfTh (original server heads)
    for cid in range(5):
        ...
        val_uni[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, uni_l2h_head, uni_rejector, val_sub,
                                              is_universal=True, client_id=cid)
        test_uni[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, uni_l2h_head, uni_rejector, tst_sub,
                                               is_universal=True, client_id=cid)
    ...
    for cid in range(5):
        ...
        _, sp_uni, _, _, _ = test_uni[cid]
        print(f"  {NAMES[cid]}: Per-Client={100*np.mean(sp_pc==y):.2f}%  Universal={100*np.mean(sp_uni==y):.2f}%")

    for rate in DEFERRAL_RATES:
        print(f"\n{'='*80}")
        print(f"  Deferral Rate = {rate}%")
        print(f"{'='*80}")
        print(f"  {'Client':<15} {'Per-L2H':>8} {'Uni-L2H':>8} {'ConfTh':>8} {'Random':>8} {'P-CT':>7} {'U-CT':>7}")
        print(f"  {'-'*15} {'-'*8} {'-'*8} {'-'*8} {'-'*8} {'-'*7} {'-'*7}")
        all_pl, all_ul, all_c, all_r = [], [], [], []
        for cid in range(5):
            # Per-client L2H
            ...
            # Universal L2H
            th_uni = find_threshold(val_uni[cid][3], rate)
            uni_acc = apply_threshold(test_uni[cid][0], test_uni[cid][1], test_uni[cid][2], test_uni[cid][3], th_uni)
            # ConfTh
            ...
            all_pl.append(pc_acc); all_ul.append(uni_acc); all_c.append(ct_acc); all_r.append(rnd)
            print(f"  {NAMES[cid]:<15} {100*pc_acc:>7.2f}% {100*uni_acc:>7.2f}% {100*ct_acc:>7.2f}% {100*rnd:>7.2f}% {100*(pc_acc-ct_acc):>+6.1f}% {100*(uni_acc-ct_acc):>+6.1f}%")
        print(f"  {'-'*15} {'-'*8} {'-'*8} {'-'*8} {'-'*8} {'-'*7} {'-'*7}")
        avg_pl, avg_ul, avg_c, avg_r = np.mean(all_pl), np.mean(all_ul), np.mean(all_c), np.mean(all_r)
        print(f"  {'AVERAGE':<15} {100*avg_pl:>7.2f}% {100*avg_ul:>7.2f}% {100*avg_c:>7.2f}% {100*avg_r:>7.2f}% {100*(avg_pl-avg_c):>+6.1f}% {100*(avg_ul-avg_c):>+6.1f}%")
