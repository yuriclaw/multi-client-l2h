#!/usr/bin/env python3
"""
v8.2e: Per-Client vs Universal with EQUAL DATA (5K each)
==========================================================
Same as v8.2d but universal uses only 5K samples (1K per client)
to make a fair data-budget comparison.
Also tests 25K for reference.
"""
...
def main():
    ...
    # 6) Per-client server heads
    print("\n[6] Training per-client server heads...")
    srv_heads = []
    for cid in range(5):
        print(f"  Client {cid}:")
        h = ServerHead(server.feat_dim, N_CLASSES)
        h = train_server_head(server, h, client_train_subs[cid])
        srv_heads.append(h)

    # 6b) Universal server heads — 5K and 25K
    all_train_indices = []
    for ci in client_train_idx:
        all_train_indices.extend(ci)

    # 5K universal: 1K per client
    uni_5k_indices = []
    for ci in client_train_idx:
        uni_5k_indices.extend(random.sample(ci, 1000))
    uni_5k_sub = Subset(train_full, uni_5k_indices)
    all_train_sub = Subset(train_full, all_train_indices)

    print("\n[6b] Training UNIVERSAL server head (5K samples)...")
    uni_srv_head_5k = ServerHead(server.feat_dim, N_CLASSES)
    uni_srv_head_5k = train_server_head(server, uni_srv_head_5k, uni_5k_sub)

    print("\n[6c] Training UNIVERSAL server head (25K samples)...")
    uni_srv_head_25k = ServerHead(server.feat_dim, N_CLASSES)
    uni_srv_head_25k = train_server_head(server, uni_srv_head_25k, all_train_sub)

    # 7) Per-client L2H rejectors (joint training)
    print("\n[7] Training per-client L2H rejectors (joint, c1=1)...")
    rejectors = []; l2h_heads = []
    for cid in range(5):
        print(f"\n  Client {cid} ({NAMES[cid]}):")
        rej = Rejector(models[cid].hidden_dim)
        rej, lh = train_rejector_joint(rej, models[cid], TEST_TFS[cid], server, srv_heads[cid],
                                        client_train_subs[cid], epochs=30, c_1=1.0)
        rejectors.append(rej); l2h_heads.append(lh)

    client_hidden_dims = {cid: models[cid].hidden_dim for cid in range(5)}

    # 8a) Universal L2H rejector — 5K data
    print("\n[8a] Training universal L2H rejector (5K, joint, c1=1)...")
    # Build 5K subsets per client (1K each)
    uni_5k_subs = [Subset(train_full, random.sample(ci, 1000)) for ci in client_train_idx]
    uni_rejector_5k = UniversalRejector(client_hidden_dims)
    uni_rejector_5k, uni_l2h_head_5k = train_universal_rejector_joint(
        uni_rejector_5k, models, TEST_TFS, server, uni_srv_head_5k,
        uni_5k_subs, client_hidden_dims, epochs=30, c_1=1.0)

    # 8b) Universal L2H rejector — 25K data
    print("\n[8b] Training universal L2H rejector (25K, joint, c1=1)...")
    uni_rejector_25k = UniversalRejector(client_hidden_dims)
    uni_rejector_25k, uni_l2h_head_25k = train_universal_rejector_joint(
        uni_rejector_25k, models, TEST_TFS, server, uni_srv_head_25k,
        client_train_subs, client_hidden_dims, epochs=30, c_1=1.0)

    # 9) Evaluate
    print("\n[9] Computing scores...")
    val_pc = {}; test_pc = {}
    val_u5 = {}; test_u5 = {}  # universal 5K
    val_u25 = {}; test_u25 = {}  # universal 25K
    val_conf = {}; test_conf = {}
    for cid in range(5):
        val_pc[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, l2h_heads[cid], rejectors[cid], val_sub)
        test_pc[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, l2h_heads[cid], rejectors[cid], tst_sub)
        val_u5[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, uni_l2h_head_5k, uni_rejector_5k, val_sub,
                                             is_universal=True, client_id=cid)
        test_u5[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, uni_l2h_head_5k, uni_rejector_5k, tst_sub,
                                              is_universal=True, client_id=cid)
        val_u25[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, uni_l2h_head_25k, uni_rejector_25k, val_sub,
                                              is_universal=True, client_id=cid)
        test_u25[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, uni_l2h_head_25k, uni_rejector_25k, tst_sub,
                                               is_universal=True, client_id=cid)
        val_conf[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, srv_heads[cid], rejectors[cid], val_sub)
        test_conf[cid] = get_scores_and_preds(models[cid], TEST_TFS[cid], server, srv_heads[cid], rejectors[cid], tst_sub)
    ...
    print("\n--- Server Accuracy ---")
    for cid in range(5):
        _, sp_pc, y, _, _ = test_conf[cid]
        _, sp_u5, _, _, _ = test_u5[cid]
        _, sp_u25, _, _, _ = test_u25[cid]
        print(f"  {NAMES[cid]}: PC(5K)={100*np.mean(sp_pc==y):.1f}%  Uni(5K)={100*np.mean(sp_u5==y):.1f}%  Uni(25K)={100*np.mean(sp_u25==y):.1f}%")

    for rate in DEFERRAL_RATES:
        print(f"\n{'='*80}")
        print(f"  Deferral Rate = {rate}%")
        print(f"{'='*80}")
        print(f"  {'Client':<15} {'Per(5K)':>8} {'Uni(5K)':>8} {'Uni(25K)':>9} {'ConfTh':>8} {'Random':>8}")
        print(f"  {'-'*15} {'-'*8} {'-'*8} {'-'*9} {'-'*8} {'-'*8}")
        all_p, all_u5, all_u25, all_c, all_r = [], [], [], [], []
        for cid in range(5):
            th_pc = find_threshold(val_pc[cid][3], rate)
            pc_acc = apply_threshold(test_pc[cid][0], test_pc[cid][1], test_pc[cid][2], test_pc[cid][3], th_pc)
            th_u5 = find_threshold(val_u5[cid][3], rate)
            u5_acc = apply_threshold(test_u5[cid][0], test_u5[cid][1], test_u5[cid][2], test_u5[cid][3], th_u5)
            th_u25 = find_threshold(val_u25[cid][3], rate)
            u25_acc = apply_threshold(test_u25[cid][0], test_u25[cid][1], test_u25[cid][2], test_u25[cid][3], th_u25)
            cth = find_conf_threshold(val_conf[cid][4], rate)
            ct_acc = apply_conf_threshold(test_conf[cid][0], test_conf[cid][1], test_conf[cid][2], test_conf[cid][4], cth)
            rnd = random_defer(test_conf[cid][0], test_conf[cid][1], test_conf[cid][2], rate)
            all_p.append(pc_acc); all_u5.append(u5_acc); all_u25.append(u25_acc); all_c.append(ct_acc); all_r.append(rnd)
            print(f"  {NAMES[cid]:<15} {100*pc_acc:>7.2f}% {100*u5_acc:>7.2f}% {100*u25_acc:>8.2f}% {100*ct_acc:>7.2f}% {100*rnd:>7.2f}%")
        print(f"  {'-'*15} {'-'*8} {'-'*8} {'-'*9} {'-'*8} {'-'*8}")
        print(f"  {'AVERAGE':<15} {100*np.mean(all_p):>7.2f}% {100*np.mean(all_u5):>7.2f}% {100*np.mean(all_u25):>8.2f}% {100*np.mean(all_c):>7.2f}% {100*np.mean(all_r):>7.2f}%")
